...
chmod +x stage.sh
stream_JBT=10.0.0.GA # staged version # a, b, c...
...
for qual in $quals; do
  for site in product central earlyaccess discovery.central discovery.earlyaccess; do
    ./stage.sh -sites $site -stream ${stream_JBT} -vr ${version} -sd 10.0 -dd static/10.0 -st staging -dt ${qual} \
      -JOB_NAME devstudio-\${stream_JBT}-build-\${site} -DESTINATION ${JBDS} -DEST_URL https://devstudio.redhat.com -q &
  done
done
...
pushd /tmp
  ...
  ./mergeISdiscovery.sh -q development -q -stable -version 10.0.0.GA -versionWithRespin 10.0.0.GA -is https://devstudio.redhat.com/9.0/staging/updates/integration-stack/discovery/9.0.0.GA/earlyaccess/ -JBDS
  rm -f /tmp/mergeISdiscovery.sh
popd
