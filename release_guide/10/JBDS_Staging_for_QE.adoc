...
cd ~/tmp
rm -f index.html
versionWithRespin_ds=10.3.0.AM2 # a, b, c...
...
# run all the .smoke jobs
# cd ~/tmp # ~
# rm -f index.html
# wget https://dev-platform-jenkins.rhev-ci-vms.eng.rdu2.redhat.com/view/Devstudio/view/devstudio_integration_tests/
# jobs=$(cat index.html  | egrep "\.smoke/build" | sed -e "s#.\+href=\"job/\(.\+\.smoke\)/build\([^\"]\+\)\".\+#\1#g") ##
# rm -f index.html

# or, just run the one buildflow job
jobs=devstudio.buildflow.it.smoke
ccijenkins=https://dev-platform-jenkins.rhev-ci-vms.eng.rdu2.redhat.com/job
JP=/tmp/jbt.github/jbosstools-build-ci/util/jenkinsPost.sh
for j in ${jobs}; do
  # ${JP} -s ${ccijenkins} -j ${j} -t enable
  ${JP} -s ${ccijenkins} -j ${j} -t buildWithParameters -d "DEVSTUDIO_VERSION=${version_ds_INST}"
  sleep 15s
