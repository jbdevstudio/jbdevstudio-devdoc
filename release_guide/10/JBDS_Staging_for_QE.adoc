...
wget https://raw.githubusercontent.com/jbosstools/jbosstools-build-ci/jbosstools-4.4.x/publish/stage.sh --no-check-certificate
chmod +x stage.sh
stream_JBT=10.0.neon # or master
versionWithRespin=10.0.0.GA-b # a, b, c...
for site in product; do
  ./stage.sh -sites $site -sd 10.0 -stream ${stream_JBT} -vr ${versionWithRespin} -JOB_NAME devstudio.\${site}_\${stream_JBT} \
...
# 5. copy devstudio discovery sites to staging/builds/ and staging/updates/
cd /tmp
if [[ -f /tmp/stage.sh ]]; then rm -f /tmp/stage.sh; fi
wget https://raw.githubusercontent.com/jbosstools/jbosstools-build-ci/jbosstools-4.4.x/publish/stage.sh --no-check-certificate
chmod +x stage.sh
...
for site in discovery.central discovery.earlyaccess; do
  ./stage.sh -sites $site -sd 10.0 -stream ${stream_JBT} -vr ${versionWithRespin} -JOB_NAME jbosstools-\${site}_\${stream_JBT} \
    -DESTINATION ${JBDS} -DEST_URL https://devstudio.redhat.com -q &
done
...
versionWithRespin=10.0.0.GA-b # a, b, c...
installerExe=development-suite-1.0.0-GA-20160616-190-bundle-installer.exe
...
http://www.qa.jboss.com/binaries/devstudio/10.0/staging/builds/devstudio-10.0.0.GA-b-build-product/latest/all/
