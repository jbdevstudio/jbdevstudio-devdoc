...
----
version=10.1.0.AM3
versionWithRespin=10.0.1.AM3 # a, b, c...
...
for targetname in jbdevstudio jbtcentral jbtearlyaccess; do
  ...
  {
    echo "[INFO] [${targetname} TP] Copy devstudio-${versionWithRespin}${suffix}.zip" | egrep "devstudio|${versionWithRespin}|${targetname}|${targetname:3}" &&
    ...
    echo "[INFO] [${targetname} TP] DONE: devstudio-${versionWithRespin}${suffix}.zip" | egrep "devstudio|${versionWithRespin}|${targetname}|${targetname:3}" &
  }
done
...
# TODO rewrite these as curl checks to see if the files exist
...
echo "https://devstudio.redhat.com/10.0/staging/updates/earlyaccess/#_____(1_zip,_1_sum,_1_folder_w/_${versionWithRespin}) " >> $tmpfile
...
versionWithRespin_PREV=10.1.0.AM3 # a, b, c...
versionWithRespin=10.0.1.AM3 # a, b, c...
...
for d in discovery.central discovery.earlyaccess; do
  ...
  pushd ${d}/${versionWithRespin}/
	...
	for c in composite*.xml; do sed -i "s/${versionWithRespin_PREV}/${versionWithRespin}/g" $c; done
