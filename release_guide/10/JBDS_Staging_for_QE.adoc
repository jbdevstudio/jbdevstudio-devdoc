...
versionWithRespin=10.0.0.Alpha2 # a, b, c...
...
for d in discovery.central discovery.earlyaccess; do
  ...
  pushd ${d}/${versionWithRespin}/
    ...
    cat composite*.xml | egrep "${versionWithRespin}|targetplatforms|REPO|updates|timestamp"
  popd
done
...
Note that these bits will still have "Alpha1" in their name, but will have a newer timestamp than the previous staged build, eg., matching 05/19 or 05/20.
...
http://www.qa.jboss.com/binaries/devstudio/10.0/staging/builds/devstudio-10.0.0.Alpha2-build-product/latest/all/
http://wonka.mw.lab.eng.bos.redhat.com/RHDS/10.0/staging/builds/devstudio-10.0.0.Alpha2-build-product/latest/all/
