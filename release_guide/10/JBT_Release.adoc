...
stream=4.4.0.Alpha1 # staged version # a, b, c...
version=4.4.0.Alpha1 # release version: no a, b, c suffix
...
for qual in $quals; do
  (( expectedCount += 18 ));
  find $HOME/TOOLS-ssh/static/neon/${qual}/updates/* -maxdepth 1 -name "*${version}*" | sort >> /tmp/${version}.found.txt
done
...
http://download.jboss.org/jbosstools/neon/development/updates/coretests/
http://download.jboss.org/jbosstools/neon/development/updates/discovery.earlyaccess/
http://download.jboss.org/jbosstools/neon/development/updates/discovery.earlyaccess/4.4.0.Alpha1/

http://download.jboss.org/jbosstools/neon/stable/updates/coretests/
http://download.jboss.org/jbosstools/neon/stable/updates/discovery.earlyaccess/
http://download.jboss.org/jbosstools/neon/stable/updates/discovery.earlyaccess/4.4.0.Alpha1/
...
pushd jbosstools-download.jboss.org/jbosstools/targetplatforms/
  ...
  for f in jbosstools; do
    pushd ${f}target
      ...
      for d in neon/composite*.xml; do
      ...
      cp -f neon/composite* ${TARGET_PLATFORM_VERSION_MAX}/
      cp -f neon/composite* ${TARGET_PLATFORM_VERSION_MAX}/REPO/
    popd
  done
  ...
  for f in jbosstools; do
    ...
    # update neon pointer
    rsync -Pzrlt --rsh=ssh --protocol=28 ${f}target/neon/* $TOOLS/targetplatforms/${f}target/neon/
  done
...
http://download.jboss.org/jbosstools/targetplatforms/jbosstoolstarget/neon/compositeContent.xml
...
# TODO SKIPPED for 4.4.0.Alpha1 because we just published bits to both paths using stage.sh so no need to move/symlink
...
pushd /tmp
  ...
  ./mergeISdiscovery.sh -q development -q -stable -version 4.4.0.Alpha1 -versionWithRespin 4.4.0.Alpha1 -is http://download.jboss.org/jbosstools/mars/staging/updates/integration-stack/discovery/4.3.0.Final/earlyaccess/
...
Create a new node on Marketplace, use content of http://download.jboss.org/jbosstools/static/neon/stable/updates/core/4.4.0.Alpha1/site.properties
...
* Update list of features, using content of http://download.jboss.org/jbosstools/static/neon/stable/updates/core/4.4.0.Alpha1/site.properties
...
* https://github.com/jbosstools/jbosstools-website/pull/568 (JBT 4.4.0.Alpha1)
...
version=4.4.0.Alpha1
eclipseVersion="Eclipse 4.6.0.M5 neon.0.M5"
