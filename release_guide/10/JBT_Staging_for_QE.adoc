...
for j in jbosstools-releng-push-to-staging-04-update-merge-composites-html; do
  ...
  ${JP} -s ${ccijenkins} -j ${j} -t buildWithParameters -d "token=RELENG&versionWithRespin_jbt=${versionWithRespin_jbt}&versionWithRespin_ds=${versionWithRespin_ds}&\
...
for j in jbosstools-releng-push-to-staging-04-update-ide-config.properties; do
  ${JP} -s ${ccijenkins} -j ${j} -t enable
  ${JP} -s ${ccijenkins} -j ${j} -t buildWithParameters -d "token=RELENG&buildType=${buildType}&\
