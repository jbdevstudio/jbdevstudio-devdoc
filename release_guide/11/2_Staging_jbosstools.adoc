...
for j in jbosstools-jiralint-weekly; do
  ${JP} -s ${ccijenkins} -c $(jp-crumb) -j ${j} -t build
done
...
version_parent=4.5.3.AM1-SNAPSHOT
...
for j in jbosstools-releng-push-to-staging-01-check-versions-branches-root-poms; do
  ...
  if [[ "${prevJob}" == "${nextJob}" ]]; then
    ...
    { ${JP} -s ${ccijenkins} -c $(jp-crumb) -j ${j} -t disable && ${JP} -s ${ccijenkins} -c $(jp-crumb) -j ${j} -t lastBuild/toggleLogKeep & }
  fi
done
...
alias jp-crumb='jenkinsURL="${ccijenkins}"; wget -q --auth-no-challenge --user nboldt --password "\$uck1tRH" --output-document - "${jenkinsURL//\/job/}/crumbIssuer/api/xml?xpath=//crumb" | sed "s#<crumb>\([0-9a-f]\+\)</crumb>#\1#"'
JP=/tmp/jbt.github/jbosstools-build-ci/util/jenkinsPost.sh
for p in aerogear base browsersim build-ci build.parent central discovery fabric8analytics forge fuse fuse-extras hibernate integration-tests.aggregate \
    javaee-tests-matrix javaee jst livereload openshift-restclient-java openshift server-Run-Unit-and-ITests server vpe webservices fuse \
    base.sonar fuse.sonar openshift.sonar server.sonar; do
...
# else
  stream_jbt=master
# fi
TARGET_PLATFORM_VERSION_MAX=4.73.0.AM2-SNAPSHOT
TARGET_PLATFORM_CENTRAL_MAX=4.72.0.Final-SNAPSHOT
...
cd ~/tmp; wget http://download.eclipse.org/technology/epp/downloads/release/oxygen/2/eclipse-jee-oxygen-2-linux-gtk-x86_64.tar.gz &
# or
...
TARGET_PLATFORM_VERSION_MAX=${TARGET_PLATFORM_VERSION_MAX}&TARGET_PLATFORM_CENTRAL_MAX=${TARGET_PLATFORM_CENTRAL_MAX}"
  ...
  if [[ "${prevJob}" == "${nextJob}" ]]; then
    ...
    ${JP} -s ${ccijenkins} -c $(jp-crumb) -j ${j} -t lastBuild/toggleLogKeep & # do not disable jobs as children may not all have started
  fi
  google-chrome ${ccijenkins}/${j} &
done
...
versionWithRespin_jbt_PREV=4.5.3.AM1 # milestones
versionWithRespin_jbt=4.5.3.AM2
versionWithRespin_jbt_NEXT=4.5.3.AM3

versionWithRespin_ds_PREV_GA=11.2.0.GA
versionWithRespin_ds_PREV=11.3.0.AM1 # milestones
versionWithRespin_ds=11.3.0.AM2
versionWithRespin_ds_NEXT=11.3.0.AM3
...
for p in aerogear base browsersim build-ci build.parent central discovery fabric8analytics forge fuse fuse-extras hibernate integration-tests.aggregate \
    ...
    base.sonar fuse.sonar openshift.sonar server.sonar; do
  j=jbosstools-${p}_master
  ${JP} -s ${ccijenkins} -c $(jp-crumb) -j ${j} -t enable &
done
echo ""
...
# else
  ...
  github_branch=master # later jbosstools-4.5.0.x
# fi
TARGET_PLATFORM_VERSION_MIN=4.71.1.Final
TARGET_PLATFORM_VERSION_MAX=4.73.0.AM2-SNAPSHOT
TARGET_PLATFORM_CENTRAL_MAX=4.72.0.Final-SNAPSHOT
respinSuffix=""
ccijenkins=https://dev-platform-jenkins.rhev-ci-vms.eng.rdu2.redhat.com/job
