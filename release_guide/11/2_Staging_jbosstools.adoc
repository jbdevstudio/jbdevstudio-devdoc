...
version_parent=4.5.2.Final-SNAPSHOT
...
for p in aerogear arquillian base browsersim build-ci build.parent central discovery fabric8analytics forge fuse fuse-extras hibernate integration-tests.aggregate \
    javaee-tests-matrix javaee jst livereload openshift-restclient-java openshift server-Run-Unit-and-ITests server vpe webservices fuse freemarker \
...
# else
  stream_jbt=master
# fi
TARGET_PLATFORM_VERSION_MAX=4.72.0.Final
...
# TODO chain these next three jobs together into a single flow, so they all run in sequence

versionWithRespin_jbt=4.5.2.Final
versionWithRespin_ds=11.2.0.GA
versionWithRespin_ds_PREV=11.2.0.GA-prev-20180118 # last staged build, not just GA
version_jbt_IS=4.5.1.Beta1b
version_ds_IS=11.1.0.Beta1b
qual_IS=development # or stable
...
ccijenkins=https://dev-platform-jenkins.rhev-ci-vms.eng.rdu2.redhat.com/job
JP=/tmp/jbt.github/jbosstools-build-ci/util/jenkinsPost.sh
for j in jbosstools-releng-push-to-staging-00-validate-composites; do
  prevJob=$(${JP} -s ${ccijenkins} -j ${j} -t enable -q); echo "[${prevJob}] ${ccijenkins}/${j} enable"
done
for j in jbosstools-releng-push-to-staging-00-validate-composites jbosstools-releng-push-to-staging-04-update-ide-config.properties; do
  prevJob=$(${JP} -s ${ccijenkins} -j ${j} -t enable -q); echo "[${prevJob}] ${ccijenkins}/${j} enable"
sleep 3s
...
Here's a job that verifies everything is updated - should automatically be triggered by the above step:

https://dev-platform-jenkins.rhev-ci-vms.eng.rdu2.redhat.com/job/jbosstools-releng-push-to-staging-00-validate-composites/
...
for j in jbosstools-releng-push-to-staging-00-validate-composites; do
  prevJob=$(${JP} -s ${ccijenkins} -j ${j} -t enable -q); echo "[${prevJob}] ${ccijenkins}/${j} enable"
sleep 3s
  nextJob=$(${JP} -s ${ccijenkins} -j ${j} -t buildWithParameters -q); echo "[${nextJob}] ${ccijenkins}/${j} buildWithParameters"
  sleep 15s
