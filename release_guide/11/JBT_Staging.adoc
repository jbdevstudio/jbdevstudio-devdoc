...
versionWithRespin_jbt=4.5.0.AM0
versionWithRespin_ds=11.0.0.AM0
...
if [[ ${version_jbt} == *".Final" ]]; then
  ...
  github_branch=master # later 4.5.x
fi
...
----
# for master branch
git checkout master; git pull origin master; git checkout -b jbosstools-4.5.x; git push origin jbosstools-4.5.x
# or, 4.5.x branch
...
cd ~/tmp; wget http://download.eclipse.org/technology/epp/downloads/release/oxygen/M5/eclipse-jee-oxygen-M5-linux-gtk-x86_64.tar.gz &
# or
...
Use devstudio@wonka.mw.lab.eng.bos.redhat.com (10.16.89.81) instead of: dev90.hosts.mwqe.eng.bos.redhat.com (10.19.65.30) or www.qa.jboss.com (10.16.89.17) as can no longer ssh to hudson@www.qa and nfs mlounted drive doesn't work anymore.
...
versionWithRespin_jbt=4.5.0.AM0
versionWithRespin_jbt_NEXT=4.5.0.AM1

versionWithRespin_ds_PREV_GA=10.3.0.GA
versionWithRespin_ds_PREV=10.4.0.GA
versionWithRespin_ds=11.0.0.AM0
versionWithRespin_ds_NEXT=11.0.0.AM1
...
if [[ ${version_jbt} == *".Final" ]]; then
  ...
  stream_jbt=4.5.oxygen
  github_branch=master # later jbosstools-4.5.x
fi
...
${JP} -s ${bosmwjenkins} -j jbosstools-releng-push-to-staging-buildflow_${stream_jbt} -t disable
