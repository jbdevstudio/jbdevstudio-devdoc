...
# note: nightly/earlyaccess includes one directory.xml file which lists both core and earlyaccess plugins, so use that
# note: may have to manually fix the list of included sites in the composite*.xml as Central (core) site content will be missing
# TODO: fix this in Jenkins to pass in 2 URLs for EXTRAS_SITE, not just 1 (so we have 4 URLs in the composite, not 3)
# TODO: update doc in ~/tru/disco/README*.adoc to explain this 
version=8.0.0.Beta2a # a, b, c...
