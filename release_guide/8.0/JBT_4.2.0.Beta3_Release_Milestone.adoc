...
hibernateToolsVersionWithRespin=4.2.0.Beta3c # it is possible that hibernate tools didn't require the same # of respins, so might have a different version
...
Then select the target release (4.2.0.Beta3 for example), type of issues = "All", style = "HTML". Copy the URL of the report page below.
...
The goal of this task is to make latest respin is available in the 'updates/development/${version}' location, without respin attribute.
...
----
  version=4.2.0.Beta3
  versionWithRespin=4.2.0.Beta3c # a, b, c, d...
  TOOLS=tools@filemgmt.jboss.org:/downloads_htdocs/tools
...
[source,bash]
----
previous=4.2.0.Beta2
versionWithRespin=4.2.0.Beta3c
eclipseTrain=luna
TOOLS=tools@filemgmt.jboss.org:/downloads_htdocs/tools
...
# TODO: make sure that JBDS is released before doing this to prevent the blog announcement from going out too early
...
*IF THIS WAS NOT ALREADY DONE* when link:JBT_4.2.0.Beta3_Staging_For_QE.adoc[Staging JBT For QE]
