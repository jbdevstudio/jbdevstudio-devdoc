...
topic=release-jbosstools-4.2.0.CR1-to-qe; branch=master; gw1

st ide-config.properties # or use another editor if not Sublime Text (st)

# replace existing lines with these to make the 4.2.0.CR1 stuff live
...
Get the file __earlyaccess.properties__ from discovery job, in workspace folder __jbosstools/org.jboss.tools.discovery.earlyaccess__,
...
version3=4.2.0.CR1 # no respin suffix here
echo "
Subject: 
