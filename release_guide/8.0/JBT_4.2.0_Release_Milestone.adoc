...
=== Make sure necessary target-platforms are released

Releases (either development or GA) are supposed to use released target-platforms. So if the build is still using a -SNAPSHOT target-platform, you should release it first following documentation at https://github.com/jbosstools/jbosstools-devdoc/blob/master/building/target_platforms/target_platforms_releases.md . An important point is that at least for GAs, the target platform site needs to leverage *Akamai* in order to improve download and installation performances. The document mentioned gives more details on how to make a target platform release site mirrored by Akamai.
