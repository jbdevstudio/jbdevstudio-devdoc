...
CAUTION: TODO: when releasing GA make sure to :%s/development/stable/g and :%s/9.0-staging/9.0/g in product/features/com.jboss.jbds.product.feature/p2.inf
...
`jboss-devstudio-9.0.0.Alpha1-v20141020-1042-B317-installer-standalone.jar` must be renamed to `jboss-devstudio-9.0.0.Alpha1-installer-standalone.jar`.
...
[1] https://access.redhat.com/jbossnetwork/restricted/listSoftware.html?downloadType=distributions&product=jbossdeveloperstudio&version=9.0.0+Alpha1
...
version=9.0.0.Alpha1
jbtVersion=4.3.0.Alpha1

for f in `echo ls | sftp $JBDS/updates/9.0.0/ 2>/dev/null | egrep "${jbtVersion}|${version}"`; do
  echo "rename updates/9.0.0/${f} static/updates/9.0.0/${f}" | sftp $JBDS
done

regularPath='../9.0.0'
akamaiPath='../../static/updates/9.0.0'
now=$(date +%s000)
wget -O compositeArtifacts.xml http://devstudio.jboss.com/updates/9.0-staging/compositeArtifacts.xml
wget -O compositeContent.xml http://devstudio.jboss.com/updates/9.0-staging/compositeContent.xml
sed --regexp-extended --in-place \
    -e s#[[:digit:]]{13}#$now## \
    -e s#$regularPath#$akamaiPath## \
    compositeArtifacts.xml compositeContent.xml
scp compositeArtifacts.xml compositeContent.jar $JBDS/updates/9.0-staging
----
...
*  9.0 for /9.0/
*  9.0-development for /9.0-development/
*  9.0-staging for /9.0-staging/
====
...
dirs="9.0-staging 9.0-development"
if [[ $isGA == "true" ]]; then dirs="9.0-staging 9.0-development 9.0"; fi

for d in $dirs; do
  if [[ $d == "9.0-staging" ]]; then
    pushd ${basedir}/updates/${d}
      ...
      for newJar in ${newJarEA} ${newJarCore}; do
        ...
        if [ "$isGA" = true ]; then
          sed -i "s#/9.0-staging/#/9.0/#g" plugin.xml
          sed -i "s#/9.0-development/#/9.0/#g" plugin.xml
        else  # plugin points to the STAGING URL, not the RELEASE one
          sed -i "s#/9.0-staging/#/9.0-development/#g" plugin.xml
          sed -i "s#/9.0/#/9.0-development/#g" plugin.xml
        fi
    ...
    pushd ${basedir}/updates/9.0-staging
      rsync -aPrz --rsh=ssh --protocol=28 ./* ${basedir}/updates/${d}/
    popd
    # now fix URLs in html pages
    pushd ${basedir}/updates/${d}/
      # set correct page label
      if [[ $d == "9.0" ]]; then sed -i "s#Development Milestone#Stable Release#" index.html; fi
      if [[ $d == "9.0-development" ]]; then sed -i "s#9.0-staging#9.0-development#" index.html; fi
      sed -i "s#9.0-staging#9.0#" index.html
    popd

  fi
done

previous=9.0.0.Alpha0
version=9.0.0.Alpha1
...
scpr jboss-devstudio-${version}*-updatesite-core.zip jboss-devstudio-${version}*-updatesite-core.zip.MD5 $JBDS/static/updates/9.0.0/
----
...
version=9.0.0.Alpha1 # name to use in filenames ie fixVersion in JIRA
versionWithRespin=9.0.0.Alpha1 # Fully qualified version, including respin suffix
...
* https://tools.jboss.org/downloads/devstudio/mars/${version}.html
* https://www.jboss.org/products/devstudio/overview/

Update site: https://devstudio.jboss.com/updates/9.0-development/

Early Access site: https://devstudio.jboss.com/earlyaccess/9.0-development/
