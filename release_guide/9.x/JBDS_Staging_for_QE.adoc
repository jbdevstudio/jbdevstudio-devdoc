...
TODO JBDS-3462: when releasing GA make sure to :%s/development/stable/g
...
for targetname in jbdevstudio jbtcentral jbtearlyaccess; do
  ...
  rsync -aPrz --rsh=ssh --protocol=28 ${y}.sha1 ${JBDS}/9.0/staging/updates/core/jboss-devstudio-${versionWithRespin}${suffix}.zip.sha1
done
...
for site in central-site earlyaccess-site; do
  ...
  if [[ $ID ]]; then
    ...
    tmpdir=/tmp/devstudio-build-sites.aggregate.${site}_${JBT_stream}__${ID}
...
y=/qa/services/http/binaries/RHDS/9.0/snapshots/builds/devstudio.product_${JBDS_stream}/${ID}/installer/jboss-devstudio-*-updatesite-core.zip
rsync -aPrz --rsh=ssh --protocol=28 ${y} ${JBDS}/9.0/staging/updates/${sitename}/jboss-devstudio-${versionWithRespin}-updatesite-core.zip
rsync -aPrz --rsh=ssh --protocol=28 ${y}.sha1 ${JBDS}/9.0/staging/updates/${sitename}/jboss-devstudio-${versionWithRespin}-updatesite-core.zip.sha1
...
for site in discovery.central discovery.earlyaccess; do
  ...
  if [[ $ID ]]; then
    if [[ ${site} == "site" ]]; then sitename="core"; else sitename=${site/-site/}; fi
    echo "Latest build for ${sitename} (${site}): ${ID}"
    tmpdir=/tmp/jbosstools-build-sites.aggregate.site_${JBT_stream}__${ID}
...
version=9.0.0.Beta1
versionWithRespin_PREV=9.0.0.Beta1b
versionWithRespin=9.0.0.Beta1c # a, b, c...
...
2. Install BYOE category from http://devstudio.redhat.com/9.0/staging/updates/ ; restart
