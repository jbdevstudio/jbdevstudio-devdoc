...
versionWithRespin=4.3.1.CR1a # a, b, c...
...
for j in jbosstools-build.parent_4.3.mars jbosstoolstargetplatforms-matrix jbosstools-buildflow_4.3.mars jbosstools-composite-install_4.3.mars jbosstools-browsersim-standalone_4.3.mars jbosstools-build-sites.aggregate.site_4.3.mars jbosstools-build-sites.aggregate.coretests-site_4.3.mars jbosstools-centraltarget_4.3.mars jbosstools-build-sites.aggregate.child-sites_4.3.mars devstudio.product_9.0.mars devstudio.versionwatch_9.0.mars jbosstools-install-p2director.install-tests.matrix_4.3.mars jbosstools-install-grinder.install-tests.matrix_4.3.mars
; do
...
versionWithRespin=CR1a # a, b, c... (not SNAPSHOT!)
TARGET_PLATFORM_VERSION_MAX=4.52.0.Final-SNAPSHOT
...
cd /tmp
if [[ -f /tmp/stage.sh ]]; rm -f /tmp/stage.sh; fi
...
# Do these steps in parallel to save time
for site in site coretests-site central-site earlyaccess-site; do
  ./stage.sh -sites $site -eclipseReleaseName mars -stream ${stream} -vr ${versionWithRespin} -JOB_NAME jbosstools-build-sites.aggregate.\${site}_\${stream} &
done
wait && echo $#

for site in browsersim-standalone; do
  ./stage.sh -sites $site -eclipseReleaseName mars -stream ${stream} -vr ${versionWithRespin} &
done
...
[source,bash]
----
version_PREV=4.3.1.Beta2 # a, b, c...
...
[source,bash]
----
versionWithRespin_PREV=4.3.1.CR1 # a, b, c...
TARGET_PLATFORM_VERSION_MAX_PREV=4.52.0.CR1-SNAPSHOT
TARGET_PLATFORM_CENTRAL_MAX_PREV=4.52.0.CR1-SNAPSHOT
...
jbdsFixVersionWithRespin=9.1.0.CR1a # a, b, c...
...
versionWithRespin=4.3.1.CR1a # a, b, c...
respin="respin-a"
TARGET_PLATFORM_VERSION_MIN=4.50.2.Final-SNAPSHOT
TARGET_PLATFORM_VERSION_MAX=4.52.0.Final-SNAPSHOT
TARGET_PLATFORM_CENTRAL_MAX=4.52.0.Final-SNAPSHOT
jbdsFixVersion=9.1.0.CR1 # no respin suffix here
